import numpy as np
from utils.helper_functions import row_col_to_seq,row_col_depth_to_seq
from utils.helper_functions import seq_to_col_row,seq_to_col_row_depths
from utils.helper_functions import prob_distortion, value_distortion, prob_normalization
...
class GridWorld:
    ...
    def add_discount(self, discount):
        ...
        """
        self.gamma = discount
    def makeblackswan(self,threshold = 0.005):
        # make rare event probabiltiy to be zero:
        for s in range(self.num_states):
            for a in range(self.num_actions):
                for s_next in range(self.num_states):
                    if self.P[s, s_next, a] >0 and self.P[s, s_next, a] < threshold :
                        self.P[s, s_next, a] = 0
                        self.blackswan.append([s, s_next, a])
                # do normalization
                self.P[s, :, a] = prob_normalization(self.P[s, :, a])

        # make rare event reward to be large as much as can.
        for pair in self.blackswan :
            s_next  = pair[1]
            self.R[s_next] = -1000

    def rdistortion(self):
    ...
    def create_gridworld(self):
        ...
        if isinstance(self.r_goal, list):
            for goal_state, r_goal in zip(self.goal_states_seq, self.r_goal) :
                self.R[goal_state] = r_goal
        else :
        ...
        for action in range(self.num_actions):
            for state in range(self.num_states):
                ...
                # else consider stochastic effects of action
                    ...
                    ## we let self. bias to be the maximum value of  ##
                    # do random sampling and set the self. biase
                    _bias = np.random.rand() * self.bias
                    ...
                    for dir in range(-1,2,1):
                        ...
                        if dir == 0:
                            ...
                            prob = (1 - self.p_good_trans) * (_bias)
                        elif dir == 1:
                            prob = (1 - self.p_good_trans)*(1-_bias)
