...
def plot_reward(reward_list,window,title,save_path):
...
def plot_reward2(reward_list1,reward_list2, label1, label2, window,title,save_path):
    ## do some moving average ##
    average_data1 ,average_data2 = [], []
    assert len(reward_list1) == len(reward_list2)
    for ind in range(len(reward_list1) - window +1 ):
        average_data1.append(np.mean(reward_list1[ind:ind+window]))
        average_data2.append(np.mean(reward_list2[ind:ind + window]))

    plt.plot(average_data1, label=label1)
    plt.plot(average_data2, label=label2)
    plt.title(title)
    plt.tight_layout()
    plt.legend()
    plt.savefig(save_path)
