...
###############################################################################
class MazeGridWorldEnv_MultiGoal(gym.Env):
    """
    ...
    def __init__(self, grid_size=(10, 10), start=(0, 0),
                 mode="hard",p_distortion=False, p_gamma=0.5,r_distortion=False,r_lambda=1):
        super(MazeGridWorldEnv_MultiGoal, self).__init__()
        ...
        # Reward
        self.success_reward_list, self.step_reward = self._define_r(r_distortion,r_lambda)
    ...
    def _define_r(self,r_distortion : bool,r_lambda : float):
        basic_success_reward_list = [3,6,9,12,15]
        basic_step_reward = -0.1
        if not r_distortion :
            print(f"success reward: {basic_success_reward_list}/ step reward: {basic_step_reward}")
            return basic_success_reward_list, basic_step_reward
        else :
            distort_success_reward_list = reward_distortion(basic_success_reward_list,r_lambda)
            distort_step_reward = reward_distortion([basic_step_reward],r_lambda)[0]
            #print(f"original success reward: {basic_success_reward_list}/ orignal step reward: {basic_step_reward}")
            print(f"distort success reward: {distort_success_reward_list}/ distort step reward: {distort_step_reward}")
            return distort_success_reward_list, distort_step_reward
    def _generate_default_maze(self, mode='hard'):
        ...
        if mode == 'easy':
            ...
            self.goal_list = [(6, 0), (1, 3), (7, 4), (2, 8), (9, 9)]
        elif mode =="nightmare2":
            walls += nightmare_internals_2
            self.goal_list = [(9, 0), (3, 6), (2, 5), (4, 8), (9, 9)]
        elif mode =="nightmare3":
            walls += nightmare_internals_3
            self.goal_list = [(4, 2), (8, 5), (1, 4), (4, 7), (9, 9)]
        else :
    ...
    def step(self, action):
        ...
        # Calculate reward
        if self.state in self.goal_list:
            # find the index
            reward_idx = self.goal_list.index(self.state)
            reward = self.success_reward_list[reward_idx]
            done = True
        else:
    ...
    def render(self,savefig=False):
        ...
        """
        # Assuming self.goal is a list of tuples [(gx1, gy1), (gx2, gy2), ...]
...
class MazeGridWorldEnv(gym.Env):
    ...
    def render(self,savefig=False):
        ...
        plt.tight_layout()
        if savefig :
            plt.savefig(f"./figures/world_{self.mode}_multigoals.pdf")
        plt.show()
